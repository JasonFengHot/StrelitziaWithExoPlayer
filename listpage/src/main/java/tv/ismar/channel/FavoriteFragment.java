...
public class FavoriteFragment extends Fragment
    ...
    public static final String MENU_TAG = "FavoriteMenu";
    ...
    private void initViews(View fragmentView) {
        ...
        clertFavorite.setOnClickListener(
                new OnClickListener() {
                    @Override
                            ...
                            if (mHGridAdapter != null) {
                                if (!isInGetFavoriteTask) {
                                    if (!IsmartvActivator.getInstance().isLogin()) {
                                        DaisyUtils.getFavoriteManager(getActivity())
                                                .deleteAll("no");
                                        reset();
                                    } else {
                                        DaisyUtils.getFavoriteManager(getActivity())
        ...
        clertFavorite.setOnFocusChangeListener(
                new View.OnFocusChangeListener() {
                    @Override
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (hasFocus) {
        ...
        clertFavorite.setOnHoverListener(
                new View.OnHoverListener() {
                    @Override
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (dialogType == AlertDialogFragment.NETWORK_EXCEPTION_DIALOG
                            ...
                            DaisyUtils.getFavoriteManager(getActivity())
                                    .deleteFavoriteByUrl((String) params[0], "no");
                            reset();
                        }
                        dialog.dismiss();
                    }
                ...
                if (mHGridAdapter != null) {
                    if (mSelectedPosition != INVALID_POSITION) {
                        ...
                        if (!isInGetFavoriteTask
                            ...
                            DaisyUtils.getFavoriteManager(getActivity())
                                    .deleteFavoriteByUrl(selectedItem.url, "no");
                            reset();
                        }
                    }
                }
    ...
    private void EmptyAllFavorite() {
        skyService
                .emptyBookmarks(IsmartvActivator.getInstance().getDeviceToken())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
    ...
    public void onDetach() {
        if (getFavoriteTask != null && !getFavoriteTask.isCancelled()) getFavoriteTask.cancel(true);
    ...
    class GetFavoriteTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            ArrayList<Favorite> favorites =
                    DaisyUtils.getFavoriteManager(getActivity()).getAllFavorites("no");
            mSectionList = new SectionList();
            FavoriteLists = new ArrayList<Item>();

            int i = 0;
            HashMap<String, ItemCollection> itemCollectionMap =
                    new HashMap<String, ItemCollection>();
            ...
            int num_pages =
                    (int)
                            Math.ceil(
                                    (float) FavoriteLists.size()
                                            / (float) ItemCollection.NUM_PER_PAGE);
            ItemCollection itemCollection =
                    new ItemCollection(num_pages, FavoriteLists.size(), "1", "1");
            mItemCollections.add(itemCollection);
            return null;
        }
        ...
        protected void onPostExecute(Void result) {
            ...
            int totalColumnsOfSectionX =
                    (int)
                            Math.ceil(
                                    (float) mItemCollections.get(mCurrentSectionPosition).count
                                            / (float) num_rows);
            //			mScrollableSectionList.setPercentage(mCurrentSectionPosition,
            // (int)(1f/(float)totalColumnsOfSectionX*100f));
        }
    }
}
