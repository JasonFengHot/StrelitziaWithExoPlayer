...
/**
 * Class to hold the media's metadata. Metadata are used for human consumption and can be embedded
 * in the media (e.g shoutcast) or available from an external source. The source can be local (e.g
 * thumbnail stored in the DB) or remote.
 *
 * <p>Metadata is like a Bundle. It is sparse and each key can occur at most once. The key is an
 * integer and the value is the actual metadata.
 *
 * <p>The caller is expected to know the type of the metadata and call the right get* method to
 * fetch its value.
 *
 * @hide
 */
public class Metadata {
     ...
     * </pre>
     *
     * @param parcel With the serialized data. Metadata keeps a reference on it to access it later
