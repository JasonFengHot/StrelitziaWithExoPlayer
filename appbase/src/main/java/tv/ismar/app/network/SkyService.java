...
public interface SkyService {
    ...
    @GET
    Observable<ItemEntity> apiItemByUrl(@Url String urlP);

    @GET
    Observable<ItemList> getItemListChannel(@Url String url);

    @GET("api/item/{pk}/")
    Observable<ItemEntity> apiItem(@Path("pk") String pk);

    @GET("api/item/{pk}/")
    Observable<OfflineCheckManager.OfflineCheckEntity> apiItemIsOffline(@Path("pk") String pk);

    @GET("api/package/{pk}/")
    Observable<OfflineCheckManager.OfflineCheckEntity> apiPKGIsOffline(@Path("pk") String pk);

    @GET("api/tv/{section}/{channel}")
    Observable<SectionList> getSectionlist(
            @Path("channel") String channel, @Path("section") String Section);

    @GET("api/tv/retrieval/{channel}")
    Observable<ResponseBody> getFilters(@Path("channel") String channel);

    @GET("api/tv/filtrate/${content_model}/{filterCondition}/1/")
    Observable<ItemList> getFilterRequest(
            @Path("content_model") String channel, @Path("filterCondition") String filterCondition);

    @GET("/api/tv/filtrate/${movie}/{area}/{page}/")
    Observable<ItemList> getFilterRequestNodata(
            @Path("movie") String movie, @Path("area") String area, @Path("page") int page);

    @GET("api/tv/filtrate/{movie}/{genre}/{page}/")
    Observable<ResponseBody> fetchFiltrate(
            @Path("movie") String movie, @Path("genre") String genre, @Path("page") String page);
    ...
    Observable<SectionList> getSections(@Path("channel") String channel);
    ...
    Observable<ResponseBody> emptyHistory(@Field("token") String token);
    ...
    Observable<ResponseBody> emptyBookmarks(@Field("token") String token);

    @GET("api/tv/relate/{pk}/")
    Observable<Item[]> getRelatedArray(@Path("pk") Integer pk);
    ...
    @GET("api/item/{pk}")
    Observable<Item> getClickItem(@Path("pk") int pk);

    @GET("api/subitem/{pk}/")
    Observable<ItemEntity> apiSubItem(@Path("pk") int pk);

    @GET("api/{opt}/{pk}/")
    Observable<ItemEntity> apiOptItem(@Path("pk") String pk, @Path("opt") String opt);

    @FormUrlEncoded
    @POST("api/bookmarks/create/")
    Observable<ResponseBody> apiBookmarksCreate(@Field("item") String item);

    @FormUrlEncoded
    @POST("api/bookmarks/remove/")
    Observable<ResponseBody> apiBookmarksRemove(@Field("item") String item);

    @GET("api/tv/relate/{pk}/")
    Observable<ItemEntity[]> apiTvRelate(@Path("pk") String item);
    ...
    Observable<ClipEntity> fetchMediaUrl(
            @Url String clipUrl, @Query("sign") String sign, @Query("code") String code);

    @FormUrlEncoded
    @POST("api/get/ad/")
    Observable<ResponseBody> fetchAdvertisement(@FieldMap HashMap<String, Object> paramsMap);
    ...
    Observable<ResponseBody> sendPlayHistory(@FieldMap HashMap<String, Object> paramsMap);

    @FormUrlEncoded
    @POST("accounts/login/")
    Observable<AccountsLoginEntity> accountsLogin(
            @Field("username") String userName, @Field("auth_number") String authNumber);

    @FormUrlEncoded
    @POST("accounts/auth/")
    Observable<ResponseBody> accountsAuth(@Field("username") String userName);
    ...
    Observable<PayLayerEntity> apiPaylayer(@Path("item_id") String itemId);

    @GET("api/paylayer/vip/{cpid}/")
    Observable<PayLayerVipEntity> apiPaylayerVip(
            @Path("cpid") String cpid, @Query("item_id") String itemId);

    @GET("api/paylayer/package/{package_id}/")
    Observable<PayLayerPackageEntity> apiPaylayerPackage(@Path("package_id") String packageId);
    ...
    Observable<ResponseBody> UploadFeedback(
            @Header("User-Agent") String userAgent, @Field("q") String q);
    ...
    @GET
    Observable<ResponseBody> openRenew(@Url String url);
    ...
    //            String accessToken);
    @GET("{geoId}.xml")
    Observable<WeatherEntity> apifetchWeatherInfo(@Path("geoId") String geoId);

    @GET
    Observable<Item> apifetchItem(@Url String url);

    @GET
    Observable<ResponseBody> apiCheckItem(@Url String url);

    @GET
    Observable<HomePagerEntity> fetchHomePage(@Url String url);

    @GET
    Observable<ItemList> getPackageList(@Url String url);

    @GET
    Observable<ItemList> getPackageListItem(@Url String url);
    ...
    Observable<List<String>> apiSearchSuggest(@Path("word") String word);

    @POST("api/tv/vodsearch/")
    Observable<VodFacetEntity> apiSearchResult(@Body VodSearchRequestEntity requestEntity);

    @GET("api/package/relate/{pkg}/")
    Observable<ItemEntity[]> packageRelate(@Path("pkg") long pk);

    @GET("api/person/{id}/")
    Observable<PersonEntitiy> apiFetchPersonBG(@Path("id") String id);
    ...
    Observable<ChatMsgEntity> Feedback(@Query("sn") String sn, @Query("topn") String topn);

    @GET("shipinkefu/getCdninfo?actiontype=getBindcdn")
    Observable<BindedCdnEntity> GetBindCdn(@Query("sn") String snCode);

    @GET("shipinkefu/getCdninfo?actiontype=bindecdn")
    Observable<Empty> BindCdn(@Query("sn") String snCode, @Query("cdn") int cdnId);

    @GET("shipinkefu/getCdninfo?actiontype=unbindCdn")
    Observable<Empty> UnbindNode(@Query("sn") String sn);
    ...
    @GET
    Observable<IpLookUpEntity> fetchIP(@Url String url);

    @GET("accounts/pay_wh_status/")
    Observable<PayWhStatusEntity> accountsPayWhStatus(@Query("pay_type") String payType);

    @GET("accounts/goods_renew_status/")
    Observable<GoodsRenewStatusEntity> accountsGoodsRenewStatus(
            @Query("package_id") int packageId, @Query("pay_type") String payType);

    @FormUrlEncoded
    @POST("accounts/open_renew/")
    Observable<OpenRenewEntity> accountsOpenRenew(
            @Field("pay_type") String payType, @Field("package_id") int packageId);

    @GET("api/agreement/")
    Observable<AgreementEntity> agreement(@Query("source") String source);
    ...
    class ServiceManager {
        ...
        private ServiceManager() {
            ...
            Retrofit speedCallaServiceRetrofit =
                    new Retrofit.Builder()
            ...
            Retrofit lilyHostServiceRetrofit =
                    new Retrofit.Builder()
            ...
            File cacheFile =
                    new File(VodApplication.getModuleAppContext().getCacheDir(), "okhttp_cache");
            ...
            OkHttpClient cacheClient =
                    new OkHttpClient.Builder()
                            ...
                            .addInterceptor(VodApplication.getHttpParamsInterceptor())
                            .addInterceptor(
                                    VodApplication.getModuleAppContext().getCacheInterceptor())
                            .addInterceptor(interceptor)
                            .addNetworkInterceptor(
                                    VodApplication.getModuleAppContext().getCacheInterceptor())
                            .cache(cache)
                            .build();
            Retrofit cacheSkyRetrofit =
                    new Retrofit.Builder()
                            .client(cacheClient)
                            .baseUrl(appendProtocol(domain[0]))
                            .addConverterFactory(GsonConverterFactory.create())
