...
public class Advertisement {
    ...
    public void fetchVideoStartAd(final ItemEntity itemEntity, final String adPid, String source) {
        ...
        mApiVideoStartSubsc =
                skyService
                        .fetchAdvertisement(getPlayerAdParam(itemEntity, adPid, source))
                        ...
                        .subscribe(
                                new Observer<ResponseBody>() {
                                    ...
                                    public void onError(Throwable e) {
                                        ...
                                        if (mApiVideoStartSubsc != null
                                                && !mApiVideoStartSubsc.isUnsubscribed()) {
                                            mApiVideoStartSubsc.unsubscribe();
                                        }
                                    }
                                    ...
                                    public void onNext(ResponseBody responseBody) {
                                        ...
                                        List<AdElementEntity> adElementEntityList =
                                                parseAdResult(result, adPid);
                                        if (adPid.equals(Advertisement.AD_MODE_ONPAUSE)) {
                                            mOnVideoPlayAdListener.loadPauseAd(adElementEntityList);
                                        } else {
                                            mOnVideoPlayAdListener.loadVideoStartAd(
                                                    adElementEntityList);
                                        }
    ...
    public void fetchAppStartAd(final String adPid) {
        ...
        mApiAppStartSubsc =
                skyService
                        .fetchAdvertisement(getAppStartParam(adPid))
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(
                                new Observer<ResponseBody>() {
                                    ...
                                    @Override
                                    public void onError(Throwable e) {
                                        if (mApiAppStartSubsc != null
                                                && !mApiAppStartSubsc.isUnsubscribed()) {
                                            mApiAppStartSubsc.unsubscribe();
                                        }
                                    ...
                                    public void onNext(ResponseBody responseBody) {
                                        ...
                                        mOnAppStartAdListener.loadAppStartAd(
                                                parseAdResult(result, adPid));
    ...
    private HashMap<String, Object> getPlayerAdParam(
            ItemEntity itemEntity, String adpid, String source) {
        ...
        adParams.put(
                "resolution",
                DeviceUtils.getDisplayPixelWidth(mContext)
                        + ","
        ...
        if (attributes != null) {
            ...
            if (directors != null) {
                for (int i = 0; i < directors.length; i++) {
                    if (i == 0) directorsBuffer.append("[");
                    directorsBuffer.append(directors[i][0]);
                    if (i >= 0 && i != directors.length - 1) directorsBuffer.append(",");
                    if (i == directors.length - 1) directorsBuffer.append("]");
                }
            }
            if (actors != null) {
                for (int i = 0; i < actors.length; i++) {
                    if (i == 0) actorsBuffer.append("[");
                    actorsBuffer.append(actors[i][0]);
                    if (i >= 0 && i != actors.length - 1) actorsBuffer.append(",");
                    if (i == actors.length - 1) actorsBuffer.append("]");
                }
            }
            if (genres != null) {
                for (int i = 0; i < genres.length; i++) {
                    if (i == 0) genresBuffer.append("[");
                    genresBuffer.append(genres[i][0]);
                    if (i >= 0 && i != genres.length - 1) genresBuffer.append(",");
                    if (i == genres.length - 1) genresBuffer.append("]");
                }
            }
        }
            ...
            if (!Utils.isEmptyText(retcode) && retcode.equals("200")) {
                ...
                for (int i = 0; i < arrays.length(); i++) {
                    ...
                    if (!Utils.isEmptyText(elementRetCode) && elementRetCode.equals("200")) {
                        AdElementEntity ad =
                                new Gson().fromJson(element.toString(), AdElementEntity.class);
                        // 已经过期的数据需要过滤掉
                        try {
                ...
                if (!adElementEntities.isEmpty()) {
                    Collections.sort(
                            adElementEntities,
                            new Comparator<AdElementEntity>() {
                                @Override
