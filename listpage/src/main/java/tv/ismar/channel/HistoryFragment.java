...
public class HistoryFragment extends Fragment
        implements ScrollableSectionList.OnSectionSelectChangedListener,
                ChannelListActivity.OnMenuToggleListener,
                MenuFragment.OnMenuItemClickedListener,
                OnItemClickListener,
                OnItemSelectedListener {

    public static final String MENU_TAG = "HistoryMenu";
    ...
    private void initViews(View fragmentView) {
        ...
        clerHistory.setOnClickListener(
                new OnClickListener() {
                    @Override
        ...
        clerHistory.setOnFocusChangeListener(
                new View.OnFocusChangeListener() {
                    @Override
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (hasFocus) {
        ...
        clerHistory.setOnHoverListener(
                new View.OnHoverListener() {
                    @Override
    ...
    private void initHistoryList() {
        ...
        mTodayItemList =
                new ItemCollection(
                        1, 0, "today", getResources().getString(R.string.vod_movielist_today));
        ...
        mYesterdayItemList =
                new ItemCollection(
                        ...
                        0,
                        "yesterday",
                        getResources().getString(R.string.vod_movielist_yesterday));
        // define early days's ItemList
        mEarlyItemList =
                new ItemCollection(
                        1, 0, "early", getResources().getString(R.string.vod_movielist_recent));
    ...
    private void addHistory(Item item) {
        ...
        else DaisyUtils.getHistoryManager(getActivity()).addHistory(history, "no", -1);
    }
    ...
    private void getHistoryByNet() {
        skyService
                ...
                .subscribe(
                        ((BaseActivity) getActivity()).new BaseObserver<Item[]>() {
                            @Override
                            ...
                            public void onNext(Item[] items) {
                                ...
                                if (items != null && items.length > 0) {
                                    ...
                                    ItemCollection itemCollection =
                                            new ItemCollection(num_pages, items.length, "1", "1");
    ...
    private void EmptyAllHistory() {
        skyService
                .emptyHistory(IsmartvActivator.getInstance().getDeviceToken())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                        ((BaseActivity) getActivity()).new BaseObserver<ResponseBody>() {
                            @Override
                            public void onCompleted() {}
    ...
    private Item getItem(History history) {
        ...
        if (history.price != 0) item.expense.price = history.price;
        if (history.cpid != 0) item.expense.cpid = history.cpid;
        if (history.cpname != null) item.expense.cpname = history.cpname;
        if (history.cptitle != null) item.expense.cptitle = history.cptitle;
        if (history.paytype != -1) item.expense.pay_type = history.paytype;
        //		}
        return item;
    }
    ...
    public void getClicItem(Item mItem) {
        ...
        int pk = 0;
        if (IsmartvActivator.getInstance().isLogin()) {
            if (mItem.model_name.equals("subitem")) pk = mItem.item_pk;
            else pk = mItem.pk;
        } else {
                ...
                .subscribe(
                        ((BaseActivity) getActivity()).new BaseObserver<Item>() {
                            ...
                            public void onNext(Item i) {
                                ...
                                if (IsmartvActivator.getInstance().isLogin())
                                    history =
                                            DaisyUtils.getHistoryManager(getActivity())
                                                    .getHistoryByUrl(url, "yes");
                                else {
                                    history =
                                            DaisyUtils.getHistoryManager(getActivity())
                                                    .getHistoryByUrl(url, "no");
                                }
                                if (history == null) {
                                    return;
                                }
                                ...
                                mDataCollectionProperties.put(
                                        "position", history.last_position / 1000);
                                String[] qualitys =
                                        new String[] {"normal", "high", "ultra", "adaptive"};
                                //	mDataCollectionProperties.put("quality",
                                // qualitys[(history.quality >= 0 && history.quality <
                                // qualitys.length) ? history.quality : 0]);
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (dialogType == AlertDialogFragment.NETWORK_EXCEPTION_DIALOG
                            ...
                            DaisyUtils.getHistoryManager(getActivity())
                                    .deleteHistory((String) params[0], "no");
                            reset();
                        }
                        dialog.dismiss();
                    }
    ...
    public void onMenuItemClicked(MenuFragment.MenuItem item) {
        ...
        switch (item.id) {
            case 1:
                if (mHGridAdapter != null && mSelectedPosition != INVALID_POSITION) {
                    ...
                    if (!isInGetHistoryTask && selectedItem.url != null) {
                        DaisyUtils.getHistoryManager(getActivity())
                                .deleteHistory(selectedItem.url, "no");
                        reset();
                    }
                }
                break;
            case 2:
                ...
                break;
            case 3:
                startSakura(getActivity());
                break;
            case 4:
                startPersoncenter(getActivity());
                break;
        }
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (i == R.id.h_grid_view) {
            ...
            int pk =
                    SimpleRestClient.getItemId(
                            tvHome.getObjects().get(position).getItem_url(), isSubItem);
            PageIntent intent = new PageIntent();
    ...
    private void getTvHome() {
        skyService
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                        ((BaseActivity) getActivity()).new BaseObserver<VideoEntity>() {
                            @Override
                            public void onCompleted() {}
        ...
        protected Void doInBackground(Void... params) {
            try {
                ...
                ArrayList<History> mHistories =
                        DaisyUtils.getHistoryManager(getActivity()).getAllHistories("no");
                ...
                if (mHistories.size() > 0) {
                    ...
                    mHistoryItemList.num_pages =
                            (int)
                                    Math.ceil(
                                            (float) mHistoryItemList.count
