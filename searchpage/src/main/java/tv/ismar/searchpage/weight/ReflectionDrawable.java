...
public class ReflectionDrawable extends Drawable {
    ...
    public ReflectionDrawable(Bitmap mBitmap, boolean isHorizontal) {
        ...
        if (isHorizontal) {
            ...
            targetBitmap =
                    Bitmap.createBitmap(
                            mBitmapWidth, (int) bitmapWithReflectionHeight, Bitmap.Config.RGB_565);
            ...
            Bitmap reflectionImage =
                    Bitmap.createBitmap(mBitmap, 0, 0, mBitmapWidth, mBitmapHeight, matrix, false);
            Bitmap bitmapWithReflection =
                    Bitmap.createBitmap(
                            mBitmapWidth,
                            (int) bitmapWithReflectionHeight,
                            Bitmap.Config.RGB_565); // 创建标准的Bitmap对象，宽和原图一致，高是原图的1.5倍
            ...
            canvas.drawRect(
                    0, 2 * mBitmapHeight, mBitmapWidth, bitmapWithReflection.getHeight(), paint);
    ...
    public void draw(Canvas canvas) {
        ...
        //        if (isHorizontal) {
        //            LinearGradient shader = new LinearGradient(0, mBitmapHeight, 0, 2 *
        // mBitmapHeight, 0x00000000, 0xff000000, Shader.TileMode.MIRROR);
        //            float bitmapWithReflectionHeight = mBitmapWidth * 355 / (float) 256;
        //            Matrix matrix = new Matrix();
        //            matrix.preScale(1, -1); // 实现图片的反转
        //            Bitmap reflectionImage = Bitmap.createBitmap(mBitmap, 0, 0, mBitmapWidth,
        // mBitmapHeight, matrix, false);
        //            Bitmap bitmapWithReflection = Bitmap.createBitmap(mBitmapWidth, (int)
        // bitmapWithReflectionHeight, Bitmap.Config.RGB_565); // 创建标准的Bitmap对象，宽和原图一致，高是原图的1.5倍
        //            Paint paint = new Paint();
        //            PorterDuffXfermode porterDuffXfermode = new
        ...
        //            paint.reset();
        //            paint.setColor(Color.BLACK);
        //            canvas.drawRect(0, 2 * mBitmapHeight, mBitmapWidth,
