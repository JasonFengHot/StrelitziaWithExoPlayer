...
public class PurchaseHistoryFragment extends BaseFragment implements PurchaseHistoryContract.View {
    ...
    public void onAttach(Activity activity) {
        ...
        View purchaseHistoryIndicator =
                mUserCenterActivity.findViewById(R.id.usercenter_purchase_history);
    ...
    public void loadAccountOrders(AccountsOrdersEntity accountsOrdersEntity) {
        ...
        ArrayList<AccountsOrdersEntity.OrderEntity> arrayList =
                new ArrayList<AccountsOrdersEntity.OrderEntity>();
        if (!TextUtils.isEmpty(IsmartvActivator.getInstance().getAuthToken())
            ...
            for (AccountsOrdersEntity.OrderEntity entity :
                    accountsOrdersEntity.getSn_order_list()) {
    ...
    private void createHistoryListView(ArrayList<AccountsOrdersEntity.OrderEntity> orderEntities) {
        ...
        for (int i = 0; i < orderEntities.size(); i++) {
            View convertView =
                    LayoutInflater.from(mUserCenterActivity)
                            .inflate(R.layout.item_purchase_history, null);
            ...
            TextView orderlistitem_remainday =
                    (TextView) convertView.findViewById(R.id.orderlistitem_remainday);
            ...
            TextView orderlistitem_paychannel =
                    (TextView) convertView.findViewById(R.id.orderlistitem_paychannel);
            ...
            String orderday =
                    mUserCenterActivity
                            .getResources()
                            .getString(R.string.personcenter_orderlist_item_orderday);
            String remainday =
                    mUserCenterActivity
                            .getResources()
            ...
            String cost =
                    mUserCenterActivity
                            .getResources()
                            .getString(R.string.personcenter_orderlist_item_cost);
            String paySource =
                    mUserCenterActivity
                            .getResources()
                            .getString(R.string.personcenter_orderlist_item_paysource);
            title.setText(item.getTitle());
            buydate_txt.setText(String.format(orderday, item.getStart_date()));
            orderlistitem_remainday.setText(
                    String.format(remainday, remaindDay(item.getExpiry_date())));
            Log.d(TAG, "remainday: " + remaindDay(item.getExpiry_date()));
            totalfee.setText(String.format(cost, item.getTotal_fee()));
            orderlistitem_paychannel.setText(
                    String.format(paySource, getValueBySource(item.getSource())));
            if (!TextUtils.isEmpty(item.getThumb_url()))
                Picasso.with(mUserCenterActivity)
            ...
            convertView.setOnClickListener(
                    new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            AccountsOrdersEntity.OrderEntity orderEntity =
                                    (AccountsOrdersEntity.OrderEntity) v.getTag();
                            ...
                            List<String> pathSegments =
                                    Uri.parse(orderEntity.getUrl()).getPathSegments();
                            String pk = pathSegments.get(pathSegments.size() - 1);
                            String type = pathSegments.get(pathSegments.size() - 2);
                            PageIntent pageIntent = new PageIntent();
                            switch (type) {
                                case "package":
                                    pageIntent.toPackageDetail(
                                            mUserCenterActivity, "history", Integer.parseInt(pk));
                                    break;
                                case "item":
                                ...
                                default:
                                    throw new IllegalArgumentException(
                                            orderEntity.getUrl() + " type not support!!!");
                            }
                        }
                    });
